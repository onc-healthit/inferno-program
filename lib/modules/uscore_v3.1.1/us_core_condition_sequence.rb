    ...
    class USCore311ConditionSequence < SequenceBase
      ...
      def perform_search_with_status(reply, search_param, search_method: :get)
        ...
        ['active', 'recurrence', 'relapse', 'inactive', 'remission', 'resolved'].each do |status_value|
          ...
          reply = get_resource_by_params(versioned_resource_class('Condition'), params_with_status, search_method: :get)
      ...
      test :search_by_patient do
        ...
        patient_ids.each do |patient|
          ...
          assert 'Device' == 'Condition' || search_with_type.length == @condition_ary[patient].length, 'Expected search by Patient/ID to have the same results as search by ID'
          ...
          assert 'Device' == 'Condition' || search_by_post_resources.length == @condition_ary[patient].length, 'Expected search by POST to have the same results as search by GET'
        end
