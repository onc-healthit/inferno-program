    ...
    class USCore311DocumentreferenceSequence < SequenceBase
      ...
      def perform_search_with_status(reply, search_param, search_method: :get)
        ...
        ['current,superseded,entered-in-error'].each do |status_value|
          ...
          reply = get_resource_by_params(versioned_resource_class('DocumentReference'), params_with_status, search_method: :get)
      ...
      test :search_by_patient do
        ...
        patient_ids.each do |patient|
          ...
          assert 'Device' == 'DocumentReference' || search_with_type.length == @document_reference_ary[patient].length, 'Expected search by Patient/ID to have the same results as search by ID'
          ...
          assert 'Device' == 'DocumentReference' || search_by_post_resources.length == @document_reference_ary[patient].length, 'Expected search by POST to have the same results as search by GET'
        end
