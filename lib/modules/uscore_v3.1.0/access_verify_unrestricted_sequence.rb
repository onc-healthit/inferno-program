...
module Inferno
  module Sequence
    class ONCAccessVerifyUnrestrictedSequence < SequenceBase
      ...
      requires :token, :patient_id, :received_scopes
      ...
      def scope_granting_access(resource, scopes)
        scopes.downcase.split(' ').find do |scope|
      ...
      test :validate_right_scopes do
        ...
        assert denied_resources.empty?, "This test requires access to all US Core resources with patient information, but the provided scope #{@instance.received_scopes} appears to grant access to all resource types."
